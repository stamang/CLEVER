...
version = "V6"
pos_fins = glob.glob("/data3/mbc/oncoshare/clever/labeled/"+version+"/neg_*.txt")
neg_fins = glob.glob("/data3/mbc/oncoshare/clever/labeled/"+version+"/pos_*.txt")
opath = "/data3/mbc/oncoshare/clever/feats/"+version+"/"
fin_pos = open(opath+"all_pos.txt","w")
fin_neg = open(opath+"all_neg.txt","w")
fout = open(opath+"clever.feats","w")
for fin in fins:
	print fin
	tmp = fin.strip(".txt")
	tmp = tmp.split("_")
